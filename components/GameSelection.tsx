...
const GameCard: React.FC<{
  ...
  >
    {icon}
    <h3
      className={`text-xl md:text-2xl font-bold ${disabled ? 'text-slate-500' : 'text-cyan-400'}`}
    >
      {title}
    </h3>
    {disabled && (
      <p className="text-yellow-400 text-xs mt-1 absolute bottom-2">
        Coming Soon!
      </p>
    )}
  </button>
...
const GameSelection: React.FC<GameSelectionProps> = ({ onSelect, onBack }) => {
  return (
    <div className="flex flex-col items-center justify-start h-screen w-full bg-slate-900 text-center relative">
      <button
        onClick={onBack}
        className="absolute top-1 left-1 md:top-4 md:left-4 text-cyan-400 hover:text-white z-20 transition-transform duration-200 hover:scale-110"
      ...
      <h1
        className="text-3xl md:text-5xl font-bold text-cyan-400 mb-3 tracking-widest"
        style={{ textShadow: '0 0 10px #06b6d4, 0 0 20px #06b6d4' }}
      >
        SELECT GAME
      </h1>
      <div className="relative w-full h-full overflow-auto">
        <div className="absolute top-0 left-0 flex flex-col w-full items-center justify-center text-center p-4">
