...
const levelConfigs = [
    { level: 1, enemies: 2, hobbins: 0, enemySpeed: 2.5, emeralds: 8, goldBags: 3, fireCooldown: 1000, rocks: 5, tunnels: 3, tunnelLength: 15 },
    { level: 2, enemies: 3, hobbins: 0, enemySpeed: 2.2, emeralds: 8, goldBags: 4, fireCooldown: 900, rocks: 7, tunnels: 3, tunnelLength: 20 },
    { level: 3, enemies: 4, hobbins: 0, enemySpeed: 2.0, emeralds: 9, goldBags: 4, fireCooldown: 850, rocks: 10, tunnels: 4, tunnelLength: 20 },
    { level: 4, enemies: 3, hobbins: 1, enemySpeed: 2.0, emeralds: 9, goldBags: 5, fireCooldown: 800, rocks: 12, tunnels: 4, tunnelLength: 25 },
    { level: 5, enemies: 4, hobbins: 1, enemySpeed: 1.8, emeralds: 10, goldBags: 5, fireCooldown: 750, rocks: 15, tunnels: 5, tunnelLength: 25 },
    { level: 6, enemies: 3, hobbins: 2, enemySpeed: 1.8, emeralds: 10, goldBags: 6, fireCooldown: 700, rocks: 18, tunnels: 5, tunnelLength: 30 },
    { level: 7, enemies: 2, hobbins: 3, enemySpeed: 1.6, emeralds: 11, goldBags: 6, fireCooldown: 650, rocks: 20, tunnels: 6, tunnelLength: 30 },
    { level: 8, enemies: 0, hobbins: 5, enemySpeed: 1.5, emeralds: 12, goldBags: 7, fireCooldown: 600, rocks: 25, tunnels: 7, tunnelLength: 35 },
];
...
export const useDiggerGame = () => {
    ...
    const [player, setPlayer] = useState<DiggerPlayerState>({ x: 0, y: 0, direction: 'RIGHT' });
    const [enemies, setEnemies] = useState<DiggerEnemyState[]>([]);
    const [goldBags, setGoldBags] = useState<DiggerGoldState[]>([]);
    const [emeraldsRemaining, setEmeraldsRemaining] = useState(0);
    ...
    const removedEmeraldsRef = useRef<Set<string>>(new Set());
    ...
    const createEmptyGrid = (): DiggerGrid => Array.from({ length: GRID_HEIGHT }, () => Array(GRID_WIDTH).fill('DIRT'));
    ...
    const generateLevel = useCallback((levelNum: number) => {
        ...
        setEmeraldsRemaining(config.emeralds);
        
        const newGoldBags: DiggerGoldState[] = [];
        for (let i = 0; i < config.goldBags; i++) {
            const pos = getValidPos('DIRT');
            newGoldBags.push({ id: getNextId(), ...pos, isFalling: false, fallTimer: 0 });
        }
    ...
    const fire = () => {
        ...
        if (now - lastFireTimeRef.current > config.fireCooldown) {
            ...
            setBullets(prev => [...prev, {id: getNextId(), x: player.x, y: player.y, direction: player.direction}]);
        }
    };
    ...
    const gameLoop = useCallback(() => {
        ...
        removedEmeraldsRef.current.clear();
        const currentCell = grid[player.y][player.x];
        const isPlayerOnLastEmerald = currentCell === 'EMERALD' && emeraldsRemaining === 1;

        if (emeraldsRemaining === 0 || isPlayerOnLastEmerald) {
             if (isPlayerOnLastEmerald) {
                 setScore(s => s + 25);
                 setEmeraldsRemaining(0);
             }
        ...
        if (enemiesToSpawnRef.current.length > 0 && gameTickCounterRef.current - lastSpawnTickRef.current > SPAWN_INTERVAL_TICKS) {
            ...
            if (tunnelCells.length > 0) {
                const spawnPos = tunnelCells[Math.floor(Math.random() * tunnelCells.length)];
        ...
        setPlayer(p => {
            const nextPos = moveEntity(p.x, p.y, playerDirectionRef.current);
            if (nextPos.x < 0 || nextPos.x >= GRID_WIDTH || nextPos.y < 0 || nextPos.y >= GRID_HEIGHT) return p;
            if (grid[nextPos.y][nextPos.x] === 'ROCK') return p;
            if (goldBags.some(bag => bag.x === nextPos.x && bag.y === nextPos.y && !bag.isFalling)) return p;
            return { ...nextPos, direction: playerDirectionRef.current };
        });

        setGrid(g => {
            const newGrid = g.map(row => [...row]);
            const cell = newGrid[player.y][player.x];
            if (cell === 'DIRT' || cell === 'EMERALD') {
                if (cell === 'EMERALD') {
                    const key = `${player.x},${player.y}`;
                    if (!removedEmeraldsRef.current.has(key)) {
                        removedEmeraldsRef.current.add(key);
                        setScore(s => s + 25);
                        setEmeraldsRemaining(e => e - 1);
                    }
                }
        ...
        setBullets(prev => {
            const nextBullets = prev.map(b => ({ ...b, ...moveEntity(b.x, b.y, b.direction) }));
            return nextBullets.filter(b => 
                b.x >= 0 && b.x < GRID_WIDTH && b.y >= 0 && b.y < GRID_HEIGHT &&
                grid[b.y]?.[b.x] !== 'DIRT' && grid[b.y]?.[b.x] !== 'ROCK'
            );
        });

        setGoldBags(bags => bags.map(bag => {
            if (bag.isFalling) {
                const nextY = bag.y + 1;
                if (nextY >= GRID_HEIGHT || grid[nextY]?.[bag.x] === 'ROCK' || grid[nextY]?.[bag.x] === 'DIRT' || grid[nextY]?.[bag.x] === 'EMERALD') {
                    setGrid(g => {
                ...
                if (bag.y + 1 < GRID_HEIGHT && grid[bag.y + 1]?.[bag.x] === 'TUNNEL') {
                    const playerBlocking = player.x === bag.x && player.y === bag.y + 1;
                    if (!playerBlocking) {
            ...
            setEnemies(prevEnemies => prevEnemies.map(enemy => {
                ...
                directions.forEach(dir => {
                    ...
                    if (next.x >= 0 && next.x < GRID_WIDTH && next.y >= 0 && next.y < GRID_HEIGHT) {
                        ...
                        if (cell === 'TUNNEL' || cell === 'GOLD' || (enemy.type === 'hobbin' && (cell === 'DIRT' || cell === 'EMERALD'))) {
                           validMoves.push(dir);
                        }
                    }
                });
                ...
                if (enemy.type === 'hobbin' && (nextCell === 'DIRT' || nextCell === 'EMERALD')) {
                    if (nextCell === 'EMERALD') {
                        const key = `${nextPos.x},${nextPos.y}`;
                        if (!removedEmeraldsRef.current.has(key)) {
                            removedEmeraldsRef.current.add(key);
                            setEmeraldsRemaining(e => Math.max(0, e - 1));
                        }
                    }
                    setGrid(g => {
                        const newGrid = g.map(r => [...r]);
        ...
        const resetPlayer = () => {
             setLives(l => {
                ...
                const respawnPos = { x: Math.floor(GRID_WIDTH / 2), y: Math.floor(GRID_HEIGHT / 2) };
    ...
    }, [isGameOver, player, grid, goldBags, bullets, level, score, generateLevel, emeraldsRemaining]);

    useInterval(gameLoop, isGameOver || isPaused ? null : tickRate);

    return { grid, player, enemies, goldBags, bullets, score, lives, level, emeraldsRemaining, isGameOver, isPaused, gameMessage, startGame, changeDirection, fire, togglePause };
};