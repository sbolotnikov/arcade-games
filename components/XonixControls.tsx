    ...
    return (
        <div className="absolute bottom-0 left-0 right-0 p-4 pb-8 flex items-center justify-center z-10 md:hidden">
            <div className="grid grid-cols-3 grid-rows-3 gap-2 w-48 h-48">
                ...
                <button onTouchStart={handlePress('LEFT')} onMouseDown={handlePress('LEFT')} disabled={isGameOver} className={buttonClasses} style={{gridColumn: 1, gridRow: 2}} aria-label="Left">
                     <ArrowIcon rotation={-90} />
                </button>
                <div />
                <button onTouchStart={handlePress('RIGHT')} onMouseDown={handlePress('RIGHT')} disabled={isGameOver} className={buttonClasses} style={{gridColumn: 3, gridRow: 2}} aria-label="Right">
                     <ArrowIcon rotation={90} />
                </button>

                <div />
                 <button onTouchStart={handlePress('DOWN')} onMouseDown={handlePress('DOWN')} disabled={isGameOver} className={buttonClasses} style={{gridColumn: 2, gridRow: 3}} aria-label="Down">
