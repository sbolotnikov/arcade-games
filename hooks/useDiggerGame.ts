...
export const useDiggerGame = () => {
    ...
    useEffect(() => {
        if (isGameOver && (gameMessage === 'GAME OVER' || gameMessage.includes('WIN'))) {
            if (score > highScore) {
    ...
    const startGame = useCallback(() => {
        setHighScore(parseInt(localStorage.getItem('digger_high_score') || '0', 10));
        setScore(0);
        setLives(3);
        setLevel(1);
        setIsGameOver(false);
    ...
    }, [isGameOver, player, grid, goldBags, bullets, level, score, highScore, generateLevel]);

    useInterval(gameLoop, isGameOver || isPaused ? null : TICK_RATE);

    return { grid, player, enemies, goldBags, bullets, score, highScore, lives, level, emeraldsRemaining, isGameOver, isPaused, gameMessage, startGame, changeDirection, fire, togglePause };
};